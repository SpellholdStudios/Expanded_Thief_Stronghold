...
IF ~Global("PGFailed","GLOBAL",1)~ THEN BEGIN 8
...
InPartySlot(LastTalkedToBy,0)~ THEN BEGIN 9
  ...
  IF ~~ THEN DO ~SetGlobal("TalkedToBrannel1","LOCALS",1)~ GOTO 10
END
...
IF ~InPartySlot(LastTalkedToBy,0)
Dead("TS5Bad")
Global("TSGTalk2","LOCALS",0)~ THEN BEGIN 14
  ...
  IF ~~ THEN DO ~SetGlobal("TSGTalk2","LOCALS",1)~ EXIT
END

IF ~InPartySlot(LastTalkedToBy,0)~ THEN BEGIN 6
  SAY #32026 /* ~Good to be seeing you again, <CHARNAME>. Go as you please, of course.~ */
  IF ~~ THEN EXIT
END

IF ~!InPartySlot(LastTalkedToBy,0)~ THEN BEGIN 7
