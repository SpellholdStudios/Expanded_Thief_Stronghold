...
IF ~Global("AmaWaukeen","GLOBAL",0)~ THEN BEGIN 0
...
IF ~~ THEN BEGIN 9
  ...
  = #33016 /* ~You are known somewhat in this place, if not as a guildmaster. He will believe you are a legitimate contact. He seems willing to believe anything, so long as it is theatrical.~ */
...
IF ~~ THEN BEGIN 12
  SAY #33023 /* ~It will be any evening that is best for you. I will await you in Waukeen's Promenade after dark, and then dispense a runner to get Sir Greshal. The suddenness will seem dramatic to him.~ */
  IF ~~ THEN GOTO 14
END
...
IF ~~ THEN BEGIN 14
  ...
  IF ~~ THEN DO ~SetGlobal("AmaWaukeen","GLOBAL",1) EscapeArea()~ EXTERN ~TSGLT2~ 7
END

IF ~Global("AmaWaukeen","GLOBAL",2)~ THEN BEGIN 15
  SAY #33030 /* ~You are here. Good. I saw you approach and have given the signal. Wait here and Sir Greshal will arrive shortly.~ */
  IF ~~ THEN DO ~SetGlobal("AmaWaukeen","GLOBAL",3)~ GOTO 17
END

IF ~!InPartySlot(LastTalkedToBy,0)~ THEN BEGIN 16
...
IF ~~ THEN BEGIN 17
  ...
  IF ~~ THEN DO ~SetGlobal("LathanPlot","GLOBAL",2) SetGlobal("AmaSetup","GLOBAL",1)~ EXIT
END

IF ~Global("AmaSetup","GLOBAL",1)~ THEN BEGIN 18
...
IF ~Global("AmaSetup","GLOBAL",2)~ THEN BEGIN 19
  SAY #33034 /* ~Ahh, here he comes now.~ */
  IF ~~ THEN DO ~SetGlobal("AmaSetup","GLOBAL",3) CreateCreature("shthoff1",[731.764],14)~ EXIT
END
...
EraseJournalEntry(55522)~ SOLVED_JOURNAL #55524 /* ~Thieves' Guild Stronghold: Ambush!
