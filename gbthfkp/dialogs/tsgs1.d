...
IF ~NumberOfTimesTalkedTo(0)~ THEN BEGIN 0
  ...
  IF ~Global("PGFailed","GLOBAL",0)~ THEN REPLY #32039 /* ~Thanks, Rattell, let's see what you have currently.~ */ DO ~StartStore("shthstor",LastTalkedToBy(Myself))~ EXIT
END
...
IF ~Global("PGFailed","GLOBAL",0)~ THEN BEGIN 2
  SAY #32040 /* ~Anything you need? Anything at all, you just ask. ~ */
  IF ~~ THEN REPLY #32041 /* ~Let me see what you have in stock.~ */ DO ~StartStore("TSStore1",LastTalkedToBy(Myself))~ EXIT
...
IF ~Global("PGFailed","GLOBAL",1)~ THEN BEGIN 4
